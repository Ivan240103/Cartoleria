  ...
  <body>
    <?php
      ...
      if ($_POST) {
        ...
        $q = "DELETE FROM articoli WHERE idArticolo = ".$_POST["art"];
        $res = mysqli_query($con, $q) or die("Errore in fase di esecuzione della query");
      }
    ?>
    <header>
      <h1>Eliminazione articolo</h1>
    </header>
    <main>
      <form action="deleteArt.php" method="post">
        <label for="art">Articolo da eliminare </label>
        <select name="art">
          <?php
            $server = "127.0.0.1";
            $user = "root";
      ...
      <?php
        ...
        if ($_POST) {
          if ($res == false) {
            echo "<p>Errore nell'eliminazione dell'articolo.</p>";
          } else {
            echo "<p>Articolo eliminato correttamente.</p>";
          }
          mysqli_close($con);
        }
      ?>
